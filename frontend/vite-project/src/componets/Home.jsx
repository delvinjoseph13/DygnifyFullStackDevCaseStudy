...
function Home() {
  ...
  const generateAndFetchData = async () => {
    ...
    try {
      setLoading(true);
      await axios.post('https://dygnifyfullstackdevcasestudy-2.onrender.com/api/generate-hazards', {
        lat: parseFloat(lat),
        lon: parseFloat(lon),
  ...
  const fetchData = async () => {
    try {
      const response = await axios.post('https://dygnifyfullstackdevcasestudy-2.onrender.com/api/hazards', {
        lat: parseFloat(lat),
        lon: parseFloat(lon),
        timeRange: { start: startYear, end: endYear }
      });
